...
import { getEmployeesJobInfoAPI, saveEmployeesJobInfoAPI } from '@/api'
...
export default {
  ...
  methods: {
    async loadEmployeesJob() {
      this.formData = await getEmployeesJobInfoAPI(this.userId)
    },
    async saveEmployeesJobs() {
      await saveEmployeesJobInfoAPI(this.formData)
